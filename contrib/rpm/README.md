...
The RPM spec file provided here is for Sucrecoin-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://sucrecoin.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Sucrecoin git repository](https://github.com/SucrecoinProject/Sucrecoin) but are not part of
...
Currently only `Source10` is used, for the example `sucrecoin.conf` file.
...
in the sucrecoin git repository. At present only an SVG version of the Sucrecoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the sucrecoin
...
needed when 0.12.1 is released, a proper fix is already in the Sucrecoin git
master branch.
...
idea to build Sucrecoin Core against a static version of that library compiled
...
The `sucrecoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `sucrecoin-server` package. At this point, most RPM based
...
The `sucrecoin-server` package should have SELinux support. How to properly do
...
Sucrecoin-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/sucrecoin/). He has been
packaging Sucrecoin for Fedora at least since 2012.
...
4. I renamed the `sucrecoin` package that contains the Qt GUI to `sucrecoin-core` as
that appears to be how the general population refers to it, in contrast to
`sucrecoin-xt` or `sucrecoin-classic`. I wanted to make sure the general population
