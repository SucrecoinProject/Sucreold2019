...
###   sucrecoincore.org and sucrecoin.org and compares them.
...
WORKINGDIR="/tmp/sucrecoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://sucrecoin.org"
HOST2="https://sucrecoin.org"
BASEDIR="/bin/"
VERSIONPREFIX="sucrecoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'sucrecoin-',
...
if [ $? -ne 0 ]; then
   echo "sucrecoin.org failed to provide signature file, but sucrecoincore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "sucrecoin.org and sucrecoincore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Sucrecoin Core binary release signing key installed?"
   fi
