...
`test/functional/test_framework/test_framework.py:SucrecoinTestFramework.main`.
...
def main():
    ...
    tmpdir = "%s/sucrecoin_test_runner_%s" % (args.tmpdirprefix, datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
    os.makedirs(tmpdir)
    ...
    enable_sucrecoind = config["components"].getboolean("ENABLE_SUCRECOIND")

    if config["environment"]["EXEEXT"] == ".exe" and not args.force:
        # https://github.com/SucrecoinProject/Sucrecoin/commit/d52802551752140cf41f0d9a225a43e84404d3e9
        # https://github.com/SucrecoinProject/Sucrecoin/pull/5677#issuecomment-136646964
        print("Tests currently disabled on Windows by default. Use --force option to enable")
        sys.exit(0)

    if not (enable_wallet and enable_utils and enable_sucrecoind):
        print("No functional tests to run. Wallet, utils, and sucrecoind must all be enabled")
...
def run_tests(test_list, src_dir, build_dir, exeext, tmpdir, jobs=1, enable_coverage=False, args=[]):
    # Warn if sucrecoind is already running (unix only)
    try:
        if subprocess.check_output(["pidof", "sucrecoind"]) is not None:
            print("%sWARNING!%s There is already a sucrecoind process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    ...
    #Set env vars
    if "SUCRECOIND" not in os.environ:
        os.environ["SUCRECOIND"] = build_dir + '/src/sucrecoind' + exeext
        os.environ["SUCRECOINCLI"] = build_dir + '/src/sucrecoin-cli' + exeext
...
class TestHandler:
    ...
    def __init__(self, num_tests_parallel, tests_dir, tmpdir, test_list=None, flags=None):
        ...
        # In case there is a graveyard of zombie sucrecoinds, we can apply a
...
class RPCCoverage():
    ...
    commands per `sucrecoin-cli help` (`rpc_interface.txt`).
