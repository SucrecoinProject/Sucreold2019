...
void static SucrecoinMiner(const CChainParams& chainparams)
{
    LogPrintf("SucrecoinMiner -- started\n");
    ...
    if (!EnsureWalletIsAvailable(pWallet, false)) {
        LogPrintf("SucrecoinMiner -- Wallet not available\n");
    }
    ...
    try {
        ...
        while (true) {
            ...
            if (!pblocktemplate.get())
            {
                LogPrintf("SucrecoinMiner -- Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
            CBlock *pblock = &pblocktemplate->block;
            IncrementExtraNonce(pblock, pindexPrev, nExtraNonce);

            LogPrintf("SucrecoinMiner -- Running miner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("SucrecoinMiner -- terminated\n");
        throw;
    }
    catch (const std::runtime_error &e)
    {
        LogPrintf("SucrecoinMiner -- runtime error: %s\n", e.what());
        return;
    }
}

int GenerateSucrecoins(bool fGenerate, int nThreads, const CChainParams& chainparams)
{
    ...
    for (int i = 0; i < nThreads; i++){
        minerThreads->create_thread(boost::bind(&SucrecoinMiner, boost::cref(chainparams)));
    }
