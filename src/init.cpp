 ...
 * between sucrecoind, and sucrecoin-qt and non-server tools.
*/
...
void Shutdown()
{
    ...
    RenameThread("sucrecoin-shutoff");
    ...
    FlushWallets();
#endif
    GenerateSucrecoins(false, 0, Params());
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), SUCRECOIN_CONF_FILENAME));
    if (mode == HMM_SUCRECOIND)
    {
#if HAVE_DECL_DAEMON
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), SUCRECOIN_PID_FILENAME));
#endif
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/SucrecoinProject/Sucrecoin>";
    const std::string URL_WEBSITE = "<https://sucrecoin.org>";
...
void ThreadImport(std::vector<fs::path> vImportFiles)
{
    ...
    RenameThread("sucrecoin-loadblk");
...
/** Sanity checks
 *  Ensure that Sucrecoin is running in a usable environment with all
...
static bool LockDataDirectory(bool probeOnly)
{
    ...
    // Make sure only a single Sucrecoin process is using the data directory.
...
bool AppInitMain(boost::thread_group& threadGroup, CScheduler& scheduler)
{
    ...
    LogPrintf("Using config file %s\n", GetConfigFile(gArgs.GetArg("-conf", SUCRECOIN_CONF_FILENAME)).string());
    ...
    GenerateSucrecoins(fGenerate, gArgs.GetArg("-genproclimit", DEFAULT_GENERATE_THREADS), chainparams);
